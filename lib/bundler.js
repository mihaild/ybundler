function get_bundling(config, args, main_generator, return_result) {
	...
	var main_generator = main_generator || 'mega';
	...
	function beautifulize_bundling(bundling) {
	...
	if (return_result) {
		return {
			bundling: beautifulize_bundling(mega.bundling),
			cost: mega.cost
		};
	}
	else {
		if (DEBUG) {
			console.log();
			console.log('mega:');
			console.log(beautifulize_bundling(mega.bundling).pages);
			console.log(mega.cost);
		}
		else {
			var bundling = beautifulize_bundling(mega.bundling);
...
exports.COA = require('coa').Cmd()
	.name(process.argv[1])
	.title('Bundler. It is better for you don\'t know what is it.')
	.helpful()
	.opt()
		.name('file_cost')
	...
	.arg()
		.name('pages')
		.title('Names of files with pages (with path). Forman of file names: [page-name].*, where page-name will be using in sessions')
		.arr()
		.val(function(page) {
